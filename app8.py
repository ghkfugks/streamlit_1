...
def main() :
    ...
    if  choice == menu[0] :
        ...
        upload_files = st.file_uploader('이미지 파일들 선택', type=['jpg','png','jpeg'],accept_multiple_files=True)


        # upload_files는 여러 파일들을 저장하고 있는 리스트
        # 어보드한 파일이 있는 경우에만 아래 코드를 실행해야한다.
    ...
    ## 데이터프레임으로 읽어서 화면에 표시!
        ...
        upload_files = st.file_uploader('CSV파일을 선택', type= ['csv'], accept_multiple_files=True)
